  ...
  const { data: addDoctor, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        "https://doctors-portal-server-eight-orcin.vercel.app/appointmentSpecialty"
      );
    ...
    fetch(url, {
      ...
      .then((imgData) => {
        ...
        if (imgData.success) {
          ...
          fetch(
            "https://doctors-portal-server-eight-orcin.vercel.app/doctors",
            {
              method: "POST",
              headers: {
      ...
      <form onSubmit={handleSubmit(handleAddDoctor)} className="px-6">
        ...
        <div className="form-control w-full">
          ...
          <input
            type="file"
            {...register("img")}
            className="border-2 border-slate-300 py-6 px-3 rounded-lg w-full"
          />
        </div>
